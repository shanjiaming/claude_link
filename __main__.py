...
def _get_env_float(name: str, default: float) -> float:
    """Read a float environment variable safely with a default.

    Returns default on any parsing error.
    """
    try:
...
def method_inject_input_to(params: Dict[str, Any]) -> Any:
    ...
    if submit:
        # Dynamically wait longer for large payloads to finish pasting.
        # Tunables:
        # - CLAUDE_LINK_BASE_DELAY_SEC (default 0.1)
        # - CLAUDE_LINK_PASTE_CHARS_PER_SEC (default 600)
        payload_length = len(payload)
        base_delay_sec = _get_env_float("CLAUDE_LINK_BASE_DELAY_SEC", 0.1)
        # Default to 600 chars/sec unless overridden
        chars_per_sec = _get_env_float("CLAUDE_LINK_PASTE_CHARS_PER_SEC", 600.0)
        if chars_per_sec <= 0:
            chars_per_sec = 600.0
        scaled_delay_sec = payload_length / chars_per_sec
        total_delay_sec = base_delay_sec + scaled_delay_sec
        if total_delay_sec < 0.1:
            total_delay_sec = 0.1
        # Allow an adjustable max cap; default 10s
        max_delay_sec = _get_env_float("CLAUDE_LINK_MAX_DELAY_SEC", 10.0)
        if total_delay_sec > max_delay_sec:
            total_delay_sec = max_delay_sec
...
def method_add_callback_hook_when_completed(params: Dict[str, Any]) -> Any:
    ...
    if mode == "text":
        ...
        cmd = f"{mcp_client} --server {_server_quoted} --method inject_text --params '{p1}' --output text"
        commands.append({"type": "command", "command": cmd})
    else:
        # mode == "screenshot": capture hooked's screenshot and inject as text to called
        # Use repr() to embed pane ids as Python string literals to avoid shell-quote issues
        py = (
            ...
            "subprocess.run(base+['--method','inject_text','--params',params,'--output','text'],check=True)"
        )
...
def method_refresh_agent_configuration(params: Dict[str, Any]) -> Any:
    """Refresh a running Claude Code agent to reload configuration.

    Preferred: use tmux respawn-pane -k to restart the pane's program in-place
    with the same pane id. Falls back to sending two Ctrl-C (0.1s apart) and
    launching 'claude --dangerously-skip-permissions -c' if respawn-pane is
    unavailable.
    """
    target_id = params.get("target_id")
    if not target_id:
    ...
    # Prefer tmux respawn-pane to avoid losing the pane target
    try:
        tm.respawn_pane(target_id, "claude --dangerously-skip-permissions -c", kill_before=True)
    except Exception:
        # Fallback: two Ctrl-C with small delays, then paste command and Enter
        tm.send_ctrl_c(target_id)
        time.sleep(0.1)
        tm.send_ctrl_c(target_id)
        time.sleep(0.1)
        cmd = "claude --dangerously-skip-permissions -c"
        buf_name = f"claude_link_refresh_{os.getpid()}_{int(time.time()*1e6)}"
        tm.set_buffer(buf_name, cmd)
        tm.paste_buffer(buf_name, target_id, delete=True)
        time.sleep(0.1)
        tm.send_enter(target_id)
    return {"ok": True}
...
_METHODS = {
    ...
    # Canonical inject_* methods; no aliases
    "inject_text": method_inject_input_to,
    "inject_keys": lambda params: (
        (lambda target_id, keys: (
            _validate_pane_id(target_id),
            tm.send_keys(target_id, keys),
            {"ok": True}
        ))(params.get("target_id"), params.get("keys") or [])
    )[-1],
    "add_callback_hook_when_completed": method_add_callback_hook_when_completed,
    "kill_pane_and_agent": method_kill_pane_and_agent,
    "refresh_agent_configuration": method_refresh_agent_configuration,
...
def _tool_specs() -> List[Dict[str, Any]]:
    ...
    return [
        ...
        {"name": "start_new_session_and_get_return_id",
         "description": "Create a new Claude Code agent (tmux pane) next to the caller; returns pane id and sets title to 'agent:<id>'. 强烈建议：如需 Hook，请在创建时通过 add_hook 一次性写入；运行中的 Claude Code 不会实时应用你之后通过 MCP 或直接编辑 .claude 的变更。对已运行的会话，任何 hooks/MCP/记忆 的更新必须调用 refresh_agent_configuration（或重启 pane）后才会生效，否则不会生效。首次创建后：若已配置 Hook，仅需调用一次 get_screenshot_from 以确认成功启动；若未配置 Hook，则需要自行主动查看进度（截图或收件箱）。注意：当 hook_mode='text' 时才需要提供 text（作为固定回调内容）；在 'screenshot' 模式会忽略 text。",
         "inputSchema": {"type": "object", "properties": {
            ...
            "calledagent": {"type": "string", "description": "Target pane id like '%7' (names may be supported by your setup)."},
             "text": {"type": "string", "description": "Only used when hook_mode='text'; fixed message inserted on callback."}
         }, "required": []}},
        {"name": "get_screenshot_from",
         "description": "Capture the full text buffer (not an image) of a target tmux pane. 主要用于确认会话已成功启动（创建/刷新后做一次）。若已配置 Hook，一次确认足够；未配置 Hook 时，用于主动查看进度（建议指数退避频率）。",
        ...
        {"name": "inject_text",
         "description": "Active delivery (forced receive): paste text exactly into target input, optionally auto-press Enter. 不做任何裁剪；使用 tmux 粘贴缓冲，保留换行/空格/ANSI。注意：当 submit=true 时，会在粘贴后自动按一次 Enter；如果你仅注入了换行且又开启 submit，会等价于双回车。仅需插入而不提交时请将 submit=false。可选 with_from/prefix；mode=append/replace。频繁注入时：若已配置 Hook，一般不需要每次截图；未配置 Hook 应采用指数退避主动查看进度。",
         "inputSchema": {"type": "object", "properties": {
        ...
        {"name": "inject_keys",
         "description": "Send raw key presses via tmux to the target pane（不做任何裁剪；适合方向键/功能键）。键名完全按 tmux 规则透传：\n- 语法：修饰符 C-/M-/S- 可叠加，如 C-M-Left；连续按可传数组 [""Down"",""Enter""]\n- 常见键：Enter/C-m、Tab/C-i、BSpace(Backspace)、DC(Delete)、Escape、Up/Down/Left/Right、Home/End、PageUp/PageDown、F1…F20\n- 示例：[""Down""], [""S-Tab""], [""C-M-Left""], [""F5""], [""Down"",""Down"",""Enter""]\n- 查键名：'man tmux' 的 KEY NAMES；或运行 'tmux list-keys -N' 参考现有名称\n- 选择建议：需要真实按键（方向/功能/组合键）用本工具；仅粘贴文本且不提交用 inject_text 并置 submit=false；只按一次回车用 [""Enter""]。",
         "inputSchema": {"type": "object", "properties": {
             "target_id": {"type": "string", "description": "Pane id like '%7'"},
             "keys": {"type": "array", "items": {"type": "string"}, "description": "tmux key names in order (e.g., 'Down', 'Enter', 'C-m', 'BSpace')"}
         }, "required": ["target_id", "keys"]}},
        {"name": "check_message_box",
         "description": "Pull inbox messages for caller pane since id. 在未配置 Hook 时用于主动拉取进度；若已配置 Hook，请避免高频轮询，可在关键节点检查或采用 1s→2s→4s 的退避策略。",
         "inputSchema": {"type": "object", "properties": {"since_id": {"type": "integer", "default": 0}}, "required": []}},
        {"name": "add_callback_hook_when_completed",
         "description": "Append a single project Stop-hook (text or screenshot) into settings.local.json with a default prefix. Use real pane ids (e.g. '%7'). 重要：对已运行的 Claude Code 注入/修改 Hook（无论通过 MCP 还是直接改 .claude）不会立即作用于当前会话；仅对后续新启动的会话生效。若要在当前会话生效，必须调用 refresh_agent_configuration（或重启 pane）。配置 Hook 后，建议仅在创建/刷新后做一次截图确认已生效；其余进度依赖 Hook 回调即可。未配置 Hook 时，需自行主动查看进度（截图或收件箱）。注意：参数 text 仅在 mode='text' 时有意义，表示固定回调文字；在 'screenshot' 模式会被忽略。",
         "inputSchema": {"type": "object", "properties": {
             ...
             "text": {"type": "string", "description": "Only used when mode='text'; fixed message inserted on callback. Ignored in 'screenshot' mode."}
        ...
        {"name": "kill_pane_and_agent",
         ...
         "inputSchema": {"type": "object", "properties": {"target_id": {"type": "string", "description": "Pane id like '%7' (names may be supported)."}}, "required": ["target_id"]}},
        {"name": "refresh_agent_configuration",
         "description": "Gracefully refresh a Claude Code agent to pick up config changes (.claude, hooks, MCP servers, memory). Uses 'tmux respawn-pane -k' to restart in-place (fallback: Ctrl-C x2 + restart). 不执行此刷新或重启，则前述更新不会生效。刷新后：若已配置 Hook，仅需截图一次确认启动；若未配置 Hook，应主动查看进度（截图或收件箱），并使用指数退避。",
         "inputSchema": {"type": "object", "properties": {"target_id": {"type": "string", "description": "Pane id to refresh (e.g., '%7')."}}, "required": ["target_id"]}},
    ]
...
def _build_stop_hook_commands_for_creation_strict(
    ...
    if mode == "text":
        ...
        cmd = f"{mcp_client} --server {_server_quoted} --method inject_text --params '{payload}' --output text"
        commands.append({"type": "command", "command": cmd})
    else:
        # Use repr() so pane ids like %34 are embedded as Python string literals
        t_lit = repr(calledagent)
        h_lit = repr(hooked_id)
        py = (
